...
test_that(".Spq_fun and .Qpq_fun return finite scalars", {
  ...
  spq <- adaptiveFTS:::.Spq_fun(dt, s = 1/4, t = 1/2, lag = 1, p = 1, q = 1,
                                bw = 0.1, kernel_name = "epanechnikov")
  qpq <- adaptiveFTS:::.Qpq_fun(dt, s = 1/4, t = 1/2, lag = 1, p = 1, q = 1,
                                bw = 0.1, bw_mean = 0.1, mean_rp = NULL,
...
test_that("estimate_mean_rp returns finite estimates and is deterministic", {
  dt <- fixture_data_far(10L)
  m1 <- estimate_mean_rp(dt, t = c(0.25, 0.5, 0.75), bw = 0.1, kernel_name = "epanechnikov")
  m2 <- estimate_mean_rp(dt, t = c(0.25, 0.5, 0.75), bw = 0.1, kernel_name = "epanechnikov")
...
test_that("estimate_autocov_rp returns finite, deterministic estimates", {
  ...
  f <- function() estimate_autocov_rp(dt, s = c(0.25, 0.5, 0.75), t = c(0.3, 0.5, 0.7),
                                      lag = 1L, bw = 0.1, bw_mean = 0.1,
                                      mean_rp = NULL, kernel_name = "epanechnikov")
  a <- f(); b <- f()
...
test_that("estimate_mean_bw_rp runs and returns a cv_error grid", {
  skip_on_cran()
  dt <- tiny_far()
  res <- estimate_mean_bw_rp(dt, n_folds = 2L, bw_grid = c(0.1, 0.2), kernel_name = "epanechnikov")
  expect_true(all(c("bw", "cv_error") %in% names(res)))
...
test_that("estimate_autocov_bw_rp runs and returns a cv_error grid", {
  ...
  res <- estimate_autocov_bw_rp(dt, n_folds = 2L, bw_grid = c(0.1, 0.2),
                                bw_mean = 0.15, mean_rp = NULL, kernel_name = "epanechnikov")
  expect_true(all(c("bw", "cv_error") %in% names(res)))
  expect_equal(nrow(res), 2L)
})
