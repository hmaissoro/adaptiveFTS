...
estimate_mean_risk <- function(data, idcol = "id_curve", tcol = "tobs", ycol = "X",
  ...
  dt_var_x <- data.table::rbindlist(lapply(#' Estimate the risk of the mean function
#'
#' @inheritParams .format_data
#' @param t \code{vector (numeric)}. Observation points at which we want to estimate the mean function of the underlying process.
#' @param bw_grid \code{vector (numeric)}. The bandwidth grid in which the best smoothing parameter is selected for each \code{t}.
...
estimate_mean_risk <- function(data, idcol = "id_curve", tcol = "tobs", ycol = "X",
  ...
  # Estimation of the variance using the presmoothing bandwidth
  dt_var_x <- data.table::rbindlist(lapply(data[, unique(id_curve)], function(idx, presmooth_bw, t, kernel_smooth, data){
    x_smooth <- estimate_nw(
      y = data[idx, X],
      t = data[idx, tobs],
      tnew = t,
      h = presmooth_bw,
      smooth_ker = kernel_smooth)
    dt_smooth <- data.table("id_curve" = idx, "t" = t, "Xtitle" = x_smooth$yhat)
    return(dt_smooth)
  }, presmooth_bw = dt_locreg[, unique(h)], t = t, kernel_smooth = smooth_ker, data = data))
  dt_var_x <- dt_var_x[! is.nan(Xtitle), .("var_x" = var(Xtitle)), by = t]

  # Estimate the risk function
  dt_Rmu <- rbindlist(lapply(bw_grid, function(h, t, H, L, kernel_smooth, data, sig_error, dt_var_x){
    # compute quantities to estimate estimate the risk
    dt_risk <- data.table::rbindlist(lapply(1:length(data), function(curve_index, t, h, H, kernel_smooth, data){
      ...
      c_n <- apply(wker, 1, function(r) sum(abs(r)))

      Tn_t_powerH <- outer(
        X = 1:length(t), Y = Tn,
        FUN = function(tid, Tnm, t, H) abs((Tnm - t[tid]) / h) ** H[tid],
        t = t, H = H
      )
      d_n_H <- diag(abs(wker) %*% t(Tn_t_powerH))
      ...
      dt_res <- data.table(
        ...
        "c_n" = c_n, "d_n_H" = d_n_H, "pi_n" = pi_n
      )
      return(dt_res)
    }, t = t, h = h, H = H, kernel_smooth = kernel_smooth, data = data))

    # Compute P_N(t, h)
    dt_risk[, P_N := sum(pi_n), by = t]

    # compute \bar C(t,h, 2H)
    dt_risk[, Cbar := (sum(pi_n * d_n_H) / P_N) ** 2, by = t]

    # Compute \mathcal N_mu(t,h) for each n = 1, ..., N
    dt_risk[, N_mu := sum(pi_n * wmax * c_n) / P_N ** 2, by = t]

    # Compute the risk for each h
    dt_risk <- unique(dt_risk[, .(t, Cbar, N_mu, P_N)])

    ## Biais term
    bias_term <- 2 * L * (h ** (2 * H)) * dt_risk[, Cbar]

    ## Variance term
    varriance_term <- 2 * (sig_error ** 2) * dt_risk[, N_mu]

    ## Convergence term to true mean
    ker_far_norm <- 0.5
    var_x <- dt_var_x[, var_x]
    dependence_coef <- 2 * max(t ** H) * ker_far_norm / (1 - ker_far_norm) ** 2
    cv_term <- 2 * sqrt(var_x) * (sqrt(var_x) + 2 * dependence_coef)  / dt_risk[, P_N]

    ## Final risk function
    R_mu <- bias_term + varriance_term + cv_term

    # Result to returned
    dt_res <- data.table("t" = t, "h" = h, "Rmu" = R_mu)
    return(dt_res)
  }, t = t, H = dt_locreg[, H], L = dt_locreg[, L], kernel_smooth = kernel_smooth, data = data,
  sig_error = sig_error, dt_var_x = dt_var_x))
...
, function(curve_index, presmooth_bw, t, kernel_smooth, data){
    x_smooth <- nw(x = data[[curve_index]][, t],
                   y = data[[curve_index]][, x],
                   xout = t,
                   h = presmooth_bw,
                   kernel = kernel_smooth)
    dt_smooth <- data.table("curve_index" = curve_index, "t" = t, "x" = x_smooth$yhat)
    return(dt_smooth)
  }, presmooth_bw = dt_locreg[, unique(h)], t = t, kernel_smooth = kernel_smooth, data = data))
