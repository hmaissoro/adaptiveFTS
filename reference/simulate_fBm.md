...
- L2:
  ...
  `float (positive)`. Squared Hölder constant \\L^2\\, the quantity the
  package's estimators report in their `Lt2`/`Ls2` columns. The Hölder
  constant itself is \\L = \sqrt{L2}\\. See the Details section.
...
The returned sample path is \\L \\ \xi(t)\\ with \\L = \sqrt{L2}\\, when
`intercept_var = 0` and `tied = FALSE`, and \\L \\ (\xi(t) + Z)\\
...
path satisfy \\E\[(X(t + \delta) - X(t))^2\] = L^2 \\ \delta^{2H}\\.
