...
estim_locreg_fun <- function(N = 400, lambda = 300, design = "d1", center = FALSE){
  if (center) {
    dt <- readRDS(paste0("./inst/11_mc_simulate_data/data/dt_mc_fma_fBm_N=", N, "_lambda=", lambda, "_centered_", design, ".RDS"))
    dt <- dt[ttag == "trandom"][, .SD, .SDcols = ! c("ttag", "fma_mean")]
  } else {
    dt <- readRDS(paste0("./inst/11_mc_simulate_data/data/dt_mc_fma_fBm_N=", N, "_lambda=", lambda, "_", design, ".RDS"))
    dt <- dt[ttag == "trandom"][, .SD, .SDcols = ! c("ttag", "fma_mean")]
  }
  ...
  ## Save
  if (center) {
    file_name <- paste0("./inst/11_mc_simulate_data/locreg_estimates/dt_locreg_fma_fBm_N=", N, "_lambda=", lambda, "_centered_", design, ".RDS")
  } else {
    file_name <- paste0("./inst/11_mc_simulate_data/locreg_estimates/dt_locreg_fma_fBm_N=", N, "_lambda=", lambda, "_", design, ".RDS")
  }
...
dt_N400_lambda300_centered_d2 <- estim_locreg_fun(N = 400, lambda = 300, design = "d2", center = TRUE)
rm(dt_N400_lambda300_centered_d2) ; gc()

### d3
dt_N400_lambda300_d3 <- estim_locreg_fun(N = 400, lambda = 300, design = "d3")
rm(dt_N400_lambda300_d3) ; gc()

dt_N400_lambda300_centered_d3 <- estim_locreg_fun(N = 400, lambda = 300, design = "d3", center = TRUE)
rm(dt_N400_lambda300_centered_d3) ; gc()
